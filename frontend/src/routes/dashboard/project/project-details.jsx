        ...
        <Tabs defaultValue="all" className="w-full">
          ...
          {['all', 'todo', 'in-progress', 'done'].map((tab) => (
            <TabsContent key={tab} value={tab} className="m-0">
              <div className={`grid ${tab === 'all' ? 'grid-cols-3' : 'md:grid-cols-1'} gap-4`}>
                {tab === 'all'
                  ...
                  : (
                    <TaskColumn
                      title={tab.replace('-', ' ')}
                      tasks={tasks.filter((task) => task.status === tab.replace('-', ' '))}
                      onTaskClick={handleTaskClick}
                      isFullWidth
                    />
                  )}
...
const TaskCard = ({ task, onClick }) => {
  return (
    <Card onClick={onClick} className="cursor-pointer hover:shadow-md transition-all duration-300 hover:translate-y-1">
      <CardHeader>
        <div className="flex items-center justify-between">
          <Badge className={task.priority === "High" ? "bg-red-500 text-white" : task.priority === "Medium" ? "bg-orange-500 text-white" : "bg-slate-500 text-white"}>{task.priority}</Badge>
          <div className="flex gap-1">
            {task.status !== "To Do" && (
              <Button variant="ghost" size="icon" className="size-6" title="Mark as To Do">
                <AlertCircle className="size-4" /><span className="sr-only">Mark as To Do</span>
              </Button>
            )}
            {task.status !== "In Progress" && (
              <Button variant="ghost" size="icon" className="size-6" title="Mark as In Progress">
                <Clock className="size-4" /><span className="sr-only">Mark as In Progress</span>
              </Button>
            )}
            {task.status !== "Done" && (
              <Button variant="ghost" size="icon" className="size-6" title="Mark as Done">
                <CheckCircle className="size-4" /><span className="sr-only">Mark as Done</span>
              </Button>
            )}
          </div>
        </div>
